...
async function main(): Promise<void> {
  ...
  try {
    ...
    const filtered = showAll ? result.deps : result.deps.filter((d) => d.status !== 'fresh')
    ...
    const nameWidth = Math.min(40, Math.max(...filtered.map((d) => d.name.length)))
    ...
    if (result.stale > 0) {
      process.stdout.write(`${STATUS_COLORS.stale}${result.stale} stale (1-2y)${RESET}  `)
    }
    const agingCount = filtered.filter((d) => d.status === 'aging').length
    if (agingCount > 0) {
      process.stdout.write(`${STATUS_COLORS.aging}${agingCount} aging (6mo-1y)${RESET}`)
    }
    process.stdout.write('\n\n')

    const exitCode = result.abandoned > 0 ? 2 : result.stale > 0 ? 1 : 0
